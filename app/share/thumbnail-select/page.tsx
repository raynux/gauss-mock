...
export default function ThumbnailSelectPage() {
  ...
  const handleNext = () => {
    if (selectedImages.length > 0) {
      router.push(`/share?images=${selectedImages.join(",")}`)
    }
  }
            ...
            {images.map((image) => (
              <div key={image.id} className="relative">
                ...
                <div className="absolute left-[-48px] top-1/2 -translate-y-1/2 flex flex-col items-center">
                  <div
                    className={`w-4 h-4 rounded-full ${selectedImages.includes(image.id) ? "bg-teal-400" : "bg-gray-200"}`}
                  ></div>
                ...
                {/* 画像カード */}
                <div
                  className={`relative rounded-lg overflow-hidden border-2 ${selectedImages.includes(image.id) ? "border-teal-400" : "border-transparent"}`}
